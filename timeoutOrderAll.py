...
def getTask(b):
    ...
    for i in range(len(b)):
        ...
        for a in tasks:
            ...
            # 创建任务对象（也可以使用字典）
            new_task = {'updateTime':updateTime,'taskCode': taskCode, 'status': status,'containerCode':containerCode,'toLocationCode':toLocationCode,'deadline':deadline,'taskType':taskType,'zone':zone,"taskPriority":taskPriority}
            # 将任务添加到新的任务列表中
...
def getoutTimeTask(c):
    ...
    for task in c:
        ...
        updateTime_time = datetime.strptime(updateTime, '%Y-%m-%d %H:%M:%S')
            # 检查更新时间是否大于当截单时间
        if updateTime_time > deadline_time :
            #转换任务状态和任务类型
    ...
    # 设置表头
    headers = ["更新时间","任务号", "状态", "容器号","工作站","截单时间","任务类型","子仓","优先级"]
...
def page(i):
    pageList = []
    for page in range(i):
        b = {"page":page+1,"size":50,"taskType":"TMS","statuses":["FINISHED"],"startCreateTime":"2024-07-11 12:00:21","endCreateTime":"2024-07-12 09:30:41"}
        pageList.append(b)
    return pageList
